    ...
    <body>
    	...
    	<div id="b1">
    		<ol class="breadcrumb" style="margin-bottom: 0px;padding-left: 72px;" id="breadcrumb">
				...
				<li style="color: #269ABC;"><span>库存查询</span></li>
			</ol>
    	</div>
    	<div id="b2">
    	...
    	<div id="b3">
    		<form id="form01" action="/crm/lx_FindAllStock.do" method="post" class="form-horizontal">
	   		<table class="table table-bordered table-hover">
	   			<tr  >
	   			<td align="center" id="label01">产品</td>
	   			<td><e:text name="sp08" styleClass="form-control" placeholder="请输入产品名称"/></td>
	   			<td align="center" id="label01">仓库</td>
	   			<td><e:text name="ck02" styleClass="form-control" placeholder="请输入仓库名"/></td>
	   			</tr>
	   		</table>
    		<table class="table table-bordered table-hover">
			<tr >
			...
			<c:choose>
				<c:when test="${not empty rows}">
					<c:forEach var="row" items="${rows}" varStatus="vs">
						<tr>
							<td>${vs.count}</td>
							<td>${row.sp08}</td>
							<td>${row.ck02}</td>
						...
						<td colspan="8" align="center">
							...
							<c:choose>
								<c:when test="${nowPage != 1}">
									<button class="btn1" formaction="/crm/lx_FindAllStock.do?nowPage=1">第一页</button>
								</c:when>
								<c:otherwise>
									第一页												
								</c:otherwise>
							...
							<c:choose>
								<c:when test="${nowPage - 1 >= 1}">
									<button class="btn1" formaction="/crm/lx_FindAllStock.do?nowPage=${nowPage - 1}">上一页</button>
								</c:when>
								<c:otherwise>上一页</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${nowPage + 1 <= pageCount}">
									<button class="btn1" formaction="/crm/lx_FindAllStock.do?nowPage=${nowPage + 1}">下一页</button>
								</c:when>
								<c:otherwise>下一页</c:otherwise>
							</c:choose>
							<c:choose>
								<c:when test="${nowPage < pageCount}">
									<button class="btn1" formaction="/crm/lx_FindAllStock.do?nowPage=${pageCount}">最后一页</button>
								</c:when>
 	...
 	<script type="text/javascript">
		function jump(){
			...
			if ((page < 1) || (page > pageCount)) {
				...
				form.action = '/crm/lx_FindAllStock.do?nowPage=' + page;
				form.submit();
			}
		}
	</script>
