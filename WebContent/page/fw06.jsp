...
<html>
	...
	<body>
		<div id="right" class="size">
			<ol class="breadcrumb" id="breadcrumb">
				...
				<form action="/crm/fw_Fw03TypeServlet.do" method="post">
					<tr>
				...
				<table id="table02" class="table table-hover table-bordered">
					...
					<c:forEach var="ibm" items="${rows }">
						<tr>
							...
							<td>${ibm.fw08 }</td>
							<td width="50px">
						<a href="/crm/page/fw_FcSelectServlet.do?fwcl_no=${ibm.fwcl_no }"  onclick="dodelete('${ibm.fw_no}')" class="input1 btn btn-sm btn-danger">⊙</a>
							</td>
						</tr>
						</c:forEach>
						<tr id="tr">
	...
	function states(){
		...
		var str=v.substr(0,1);
		if(str!=3){
			window.location.href="/crm/fw_Fw03TypeServlet.do?type=state&no="+str;
		}else{
			window.location.href="/crm/fw_Fw03Servlet.do";
		}
	}
	function cs(){
		...
		if(str!=4){
			window.location.href="/crm/fw_Fw03TypeServlet.do?type=type&no="+str;
		}else{
			window.location.href="/crm/fw_Fw03Servlet.do";
		}
	}
	 function delect(obj,fw_no){
		 var index = obj.parentNode.rowIndex;//这是获取的是《tr》标签
		var table=document.getElementById('table02');  //这是获取的table然后赋值给var table
		/*   table.deleteRow(index);  
		window.location.href("DelectServlet?fw_no="+fw_no); */
		//window.location.href("FW03AddCLServlet.do?fw_no"+fw_no);
		window.location.href("FW03AddCLServlet?fw_no="+fw_no);
